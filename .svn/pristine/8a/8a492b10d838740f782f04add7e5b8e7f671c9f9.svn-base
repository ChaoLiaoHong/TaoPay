 ...
 * 若不需要Lazy加载则initData方法内留空,初始化内容放到initView即可,想每次可见都加载调用refreshOnVisible
 * -
...
public abstract class BaseFragment extends Fragment {
    ...
    private void refreshOnUserVisible() {
    ...
    /**
     * 每次用户可见的时候 , 如果是想只加载一次就用initData , 这个是每次切换到fragment都加载
     */
    ...
    public String getHttpMsg(String response) {
        if (!AppUtils.checkId(response)) return "";
        String msg = "";
        try {
    ...
    public long getMemberId() {
        return (Long) SharedPreferencesUtil.get(context, "memberId", 0l);
    }
}
