...
public class TimeUtil {
     ...
     * @param ldt  传入值要是毫秒精度
     *             时间戳
     * @param form MM-dd HH:mm:ss 格式化字符串 可为空
     * @return
     */
    public static String getTimeStringMD(long ldt, String form) {
        String localform = form;
        if ("".equals(localform)) {
            localform = "MM-dd HH:mm:ss";
        }
        Date dt = new Date(ldt);
        SimpleDateFormat time = new SimpleDateFormat(localform);
        return time.format(dt);
    }
    public static String getTime3String(long ldt, String form) {
        String localform = form;
        if ("".equals(localform)) {
            localform = "yyyy-MM-dd";
        }
        Date dt = new Date(ldt);
        SimpleDateFormat time = new SimpleDateFormat(localform);
        return time.format(dt);
